...
from .modules import DFL #, C2f, RepConv, Proto, Detect, Segment, Pose, OBB, DSConv, v10Detect
...
class Bottleneck_IRFAConv(Bottleneck):
    """Standard bottleneck with IRFAConv."""

    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = IRFAConv(c_, c2, k[1])

class CSRFA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_IRFAConv(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class IRFAConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size,stride=1):
        super().__init__()
        self.k = kernel_size
        self._φ = lambda: nn.Unfold(kernel_size=(self.k, self.k), padding=self.k // 2)
        self.unfold = self._φ()
        self._ω = lambda ch, ks: nn.Sequential(
            nn.Conv2d(ch * (ks ** 2), ch * (ks ** 2), kernel_size=1, groups=ch),
            nn.BatchNorm2d(ch * (ks ** 2))
        )
        self.get_weights = self._ω(in_channel, kernel_size)
        self._κ = lambda ic, oc, ks: (
            nn.Conv2d(ic, oc, kernel_size=ks, padding=0, stride=ks),
            nn.BatchNorm2d(oc),
            nn.ReLU()
        )
        self.conv, self.bn, self.act = self._κ(in_channel, out_channel, kernel_size)
        
    def forward(self, x):
        b, c, h, w = x.shape
        ψ = lambda t: t.unsqueeze(-1)
        ξ = self.unfold(x)
        θ = self.get_weights(ψ(ξ)).view(b, c, self.k ** 2, h, w).permute(0, 1, 3, 4, 2).softmax(-1)
        η = rearrange(θ, 'b c h w (n1 n2) -> b c (h n1) (w n2)', n1=self.k, n2=self.k)
        μ = rearrange(ξ, 'b (c n1) l -> b c n1 l', n1=self.k ** 2).permute(0, 1, 3, 2).reshape(b, c, h, w, self.k ** 2)
        ν = rearrange(μ, 'b c h w (n1 n2) -> b c (h n1) (w n2)', n1=self.k, n2=self.k)
        return self.act(self.bn(self.conv(ν * η)))
